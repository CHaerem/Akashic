...
struct PlaceEnrichment: Sendable {
    ...
    /// Ordered natural-language queries used to find a *named natural feature* near a day when the
    /// reverse-geocode has no area of interest — the "Cuzco ×5" case, where the locality alone is
    /// editorially dead. Ordered by how evocative the hit tends to be; the first query that yields a
    /// named result wins (we don't run the rest). A named summit/lake/park beats "Cuzco".
    static let campFeatureQueries: [String] = ["national park", "mountain", "summit", "lake"]

    /// Small radius for the named-feature search — deliberately tight so the feature genuinely
    /// belongs to *this* day rather than a distant landmark that happens to share the region.
    static let campFeatureRadiusMeters: Double = 1500
    ...
    ///
    /// Naming preference (see `campNameCandidates`): a named **area of interest** first, then the
    /// nearest **named natural feature/POI** from a tight-radius local search, and only then the
    /// bare **locality**. This is what turns five identical "Cuzco" rows into "Machu Picchu",
    /// "Wiñay Wayna", … — a locality is technically right but editorially dead when repeated.
    ///
    /// **Consecutive-day de-dupe:** a day is never offered a name equal to the PREVIOUS day's
    /// suggestion. When the top candidate repeats the last one we fall through to the next candidate,
    /// and if every candidate repeats it, that day is skipped (no suggestion) rather than adding
    ...
    func suggestCampNames(for days: [DayEnrichmentInput]) async -> [CampNameSuggestion] {
        ...
        var lastSuggested: String?     // lowercased name emitted for the previous day, to avoid repeats
        var owesDelay = false          // whether a courtesy delay is owed before the next provider call
        for day in days {
            guard DayNamer.isAutoGenerated(name: day.name), day.hasCoordinate else { continue }
            if owesDelay { try? await Task.sleep(nanoseconds: interCallDelayNanos) }
            owesDelay = true
            guard let place = try? await geocoder.reverseGeocode(lng: day.coordinate[0], lat: day.coordinate[1])
            else { continue }
            // Only reach for a nearby named feature when the placemark itself offers no area of
            // interest — that's exactly the case where locality would otherwise be all we have.
            var feature: String?
            let hasAreaOfInterest = !(place.areaOfInterest?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ?? true)
            if !hasAreaOfInterest {
                feature = await nearestNamedFeature(lng: day.coordinate[0], lat: day.coordinate[1])
            }
            let candidates = Self.campNameCandidates(place: place, feature: feature)
            // First candidate that isn't a repeat of the previous day's suggestion; skip the day
            // entirely if every candidate would repeat it.
            guard let pick = candidates.first(where: { $0.lowercased() != lastSuggested }) else { continue }
            lastSuggested = pick.lowercased()
            out.append(CampNameSuggestion(dayID: day.dayID, name: pick))
        }
        return out
    }

    /// The ordered day-name candidates for one placemark: area of interest, then a nearby named
    /// natural feature (when supplied), then locality → sub-locality → region → placemark name.
    /// Pure and case-insensitively de-duplicated (a `name` that merely echoes the locality is
    /// dropped), so the preference order is unit-tested without any provider.
    static func campNameCandidates(place: GeocodedPlace, feature: String? = nil) -> [String] {
        var out: [String] = []
        for candidate in [place.areaOfInterest, feature, place.locality,
                          place.subLocality, place.administrativeArea, place.name] {
            guard let trimmed = candidate?.trimmingCharacters(in: .whitespacesAndNewlines),
                  !trimmed.isEmpty,
                  !out.contains(where: { $0.lowercased() == trimmed.lowercased() })
            else { continue }
            out.append(trimmed)
        }
        return out
    ...
    /// The nearest **named** natural feature/POI within `campFeatureRadiusMeters` of a coordinate,
    /// or nil. Runs `campFeatureQueries` serially and stops at the first query that yields any named
    /// result, choosing the geographically nearest hit within it (results without a coordinate are
    /// treated as closest). Every failure degrades to nil — a missing feature is never an error.
    private func nearestNamedFeature(lng: Double, lat: Double) async -> String? {
        for query in Self.campFeatureQueries {
            try? await Task.sleep(nanoseconds: interCallDelayNanos) // courtesy delay before each serial call
            let results = (try? await search.search(query: query, lng: lng, lat: lat,
                                                    radiusMeters: Self.campFeatureRadiusMeters)) ?? []
            var best: String?
            ...
            for result in results {
                ...
                let km: Double
                if let coord = result.coordinate, coord.count >= 2 {
                    km = PhotoDayMatcher.distanceKm(lat, lng, coord[1], coord[0])
                } else {
                    km = 0
                }
                if km < bestKm { bestKm = km; best = name }
            }
            if let best { return best }   // ordered queries: first one with a named hit wins
        }
        return nil
    }
