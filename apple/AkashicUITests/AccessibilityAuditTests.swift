...
final class AccessibilityAuditTests: AkashicUITestCase {
    ...
    ///     findings on all seven screens below, which is that pass finally checked rather than
    ///     asserted.
    ...
    ///   * `.hitRegion` — a target under 44 × 44 pt. Found four real defects on the paywall,
    ///     including "Restore purchases" at 18 pt tall; all four are fixed in `PaywallView`.
    ...
    ///
    /// `.contrast`, `.dynamicType` and `.textClipped` produce **123 findings across these eight
    /// audited hierarchies**. They are not 123 bugs — they are three systemic design decisions, each
    /// traceable to one seam and each measured:
    ///
    ///   * **contrast (61: 32 outright, 29 large-text-only)** is `Theme.textSecondary` and
