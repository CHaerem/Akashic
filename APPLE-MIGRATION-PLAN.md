...
| D4 | **Core Data + `NSPersistentCloudKitContainer` + CKShare** (default) or **`CKSyncEngine`** (alternative) for the native sync layer | SwiftData does not support shared databases today. NSPCKC is the mature, documented path with sharing support; CKSyncEngine (iOS 17+) is cleaner but hand-rolled. Decide after the Phase 0 spike. Revisit SwiftData if Apple adds sharing |
| D5 | **MapKit with realistic elevation** on iOS, verify globe experience in a spike; fall back to Mapbox iOS SDK (free tier: 25k MAU) if the zoomed-out globe disappoints | Removes token dependency if MapKit suffices; the rotating globe is the app's signature and must not regress |
