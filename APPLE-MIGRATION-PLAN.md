...
| D6 | **✅ CONFIRMED (2026-07-22): Web = read-mostly CloudKit JS client, scope FROZEN** at viewing + light edits (captions/comments) — every other capability is native-only. Private-DB reads proven live in the browser (Spike A + the app's adapter against the real archive); shared-DB reads + share acceptance remain the one unproven piece, verified as part of T2.8. Desktop on Apple hardware is served by the iOS app itself via TestFlight for Mac (Apple Silicon) — the web client's remaining jobs are Windows/any-browser access and the shareable akashic.no face. **Re-evaluation point:** after ~1 month of real family use, usage decides whether the client stays, slims to the public showcase (D9), or retires — both outcomes are deletions, not builds. MapKit JS has no globe as of v6 (June 2026); Mapbox GL JS stays on web, re-checked each WWDC (Phase 6 watch item) | CloudKit JS supports Apple ID sign-in, private *and* shared DB access, and share acceptance — but the library is aging. Minimizing the web surface keeps risk low while keeping akashic.no alive |
