...
| `QUA-91` SwiftUI Map declutters camp badges on its own — only 3 of 8 are reachable on the Kilimanjaro overview | claude-remote | `claude/remote-control-d38598` | CORRECTION to the previous note, measured not inferred. metersPerPoint is 38.06 m/pt on the Kilimanjaro overview (probe AKASHIC_MAP_PROBE=1, now asserted by testTheLiveProjectionIsEstablishedOnTheOverview). So the projection IS established, campGroups(for:) DOES take its live branch, and QUA-83's photo clearance DOES run -- the previous note's claim that metersPerPoint was 0, and the two consequences drawn from it, are refuted. Why 44 pt and 96 pt looked identical: only SURVIVING annotations appear in the tree, and the survivors (camps 1, 2, 7) are singletons at either threshold. At 96 pt the middle camps did merge and MapKit culled the merged badge anyway; Barranco and Barafu are 4.4 km apart, about 116 pt at this framing, and are still culled. OPEN QUESTION for whoever takes this: MapKit's culling is either more aggressive than any pairwise separation that keeps day-level framing usable, or it is not distance-based at all. Next step is a threshold sweep to find the floor, or a different shape entirely (fewer annotations: draw camps into the polyline layer, or one badge per visible screen region). The reusable lesson: two rounds of confident reasoning from symptoms were both wrong and one probe settled it. |
